...
# pins is a dictionary that connects the RGB pins of the LED to 
# three different pins to the Rasp pi. I am using GPIO17, GPIO18,
# and GPIO27.
...
def setup():
  ...
  p_R = GPIO.PWM(pins['Red'], 2000)     # pin to PWM at freq of 2000hz
...
def main():
  while True:
    for color in COLOR: # Assign every item in the COLOR list to the color 
                        # respectively and change the color of the RGB LED 
                        # via the setColor() function.

      setColor(color)   # change the color of the RGB LED

      time.sleep(0.5)   # set delay for 0.5s after each color changing. 
                        # Modify this parameter will changed the LED's 
                        # color changing rate.   
