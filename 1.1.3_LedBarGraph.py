...
def oddLedBarGraph():
  for i in range(5):
    j = i*2         #every even index of ledPins. Since index 
                    # starts at 0, that means every odd pin.
...
def evenLedBarGraph():
  for i in range(5):
    j = i*2+1       #every odd index of ledPins. Since index
                    # starts at 0, that means every even pin.
    GPIO.output(ledPins[j],GPIO.HIGH)
    time.sleep(0.3)
    GPIO.output(ledPins[j],GPIO.LOW)
